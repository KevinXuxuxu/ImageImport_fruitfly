...
I = im2double(imread(names(i).name));
...
gcs = computeSPGravityCenters(Sp2);
save(strcat('output/', names(i).name, '.mat'), 'gcs');
boundaries = cell(length(gcs),1);
for j=1:length(gcs)
    boundaries{j} = getBoundary(Sp2, j);
end
save(strcat('output/', names(i).name, '.mat'), 'boundaries', '-append');
end
